  ...
  flows: [
    {
      ...
      steps: [
        ...
        {
          label: "Map processes to the ROS graph",
          detail:
            "Linux processes resolved to nodes, domains, PIDs, executables and workspaces.",
        },
        {
          label: "Is a node actually alive?",
          tone: "decision",
          branches: [
            {
              label: "Process gone but topic held, zombie detected",
              tone: "bad",
            },
            {
              label: "Two nodes claiming the same name, conflict detected",
              tone: "bad",
            },
        ...
        {
          label: "Inspect topics, QoS, TF, Nav2 and lifecycle state",
          tone: "decision",
          branches: [
            {
              label: "QoS mismatch, publisher and subscriber never connect",
              tone: "bad",
            },
            {
              label: "Missing TF link, the transform tree is incomplete",
              tone: "bad",
            },
        ...
        {
          label: "Snapshot or record the session",
          detail:
  ...
  tables: [
    {
      ...
      rows: [
        [
          "Zombie process",
          "The process is gone but its topic or service registration is still held",
        ],
        ["Name conflict", "Two nodes claiming the same identity in the graph"],
        [
          "Missing link",
          "A topic with a publisher and no subscriber, or the reverse",
        ],
        [
          "QoS mismatch",
          "Endpoints that will never connect because their policies are incompatible",
        ],
    ...
    },
    {
      ...
      rows: [
        [
          "Engineer",
          "Debugging",
          "Full graph, QoS, TF, lifecycle, process control, snapshots, SSH fleet monitoring",
        ],
        [
          "Operator",
          "The floor",
          "A simplified runtime and safety view, with the diagnostics out of the way",
        ],
      ],
    },
  ],
